...
public:
    ...
    ros::Publisher pubCloudRegisteredLocal; // 发布局部坐标系下点云
    ...
    mapOptimization()
    {
        ...
        pubCloudRegisteredLocal = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/cloud_registered_local", 1);
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void publishOdometry()
    {
        ...
        // static int frame_id = 0; // 当前帧的id号
        // std::string file_name = "/media/jh/KESU/Data/zeus/pose_pcd/seq_2/" + std::to_string(frame_id) + ".txt";
        // pose_file.open(file_name);
        // pose_file << "# Pose: ts, tx, ty, tz, qx, qy, qz, qw" << std::endl;

        // pose_file << std::fixed << std::setprecision(5) << laserOdometryROS.header.stamp.toSec() << " " << laserOdometryROS.pose.pose.position.x << " "
        //           << laserOdometryROS.pose.pose.position.y << " " << laserOdometryROS.pose.pose.position.z << " " << laserOdometryROS.pose.pose.orientation.x << " "
        //           << laserOdometryROS.pose.pose.orientation.y << " " << laserOdometryROS.pose.pose.orientation.z << " " << laserOdometryROS.pose.pose.orientation.w << std::endl;
        ...
        // pcl::io::savePCDFileBinary("/media/jh/KESU/Data/zeus/pose_pcd/seq_2/" + std::to_string(frame_id) + ".pcd", *cloudOut);
        // frame_id++;
        // t2 = ros::Time::now();

        // double t = (t2 - t1).toSec();
    ...
    void publishFrames()
    {
        ...
        sensor_msgs::PointCloud2 cloud_local_msg = cloudInfo.cloud_deskewed;
        cloud_local_msg.header.frame_id = lidarFrame;
        cloud_local_msg.header.stamp = timeLaserInfoStamp;

        pubCloudRegisteredLocal.publish(cloud_local_msg);
        // pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
        // pcl::fromROSMsg(cloudInfo.cloud_deskewed, *cloudOut);
    }
};
