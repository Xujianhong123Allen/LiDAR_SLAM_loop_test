...
void LidarIris::initial(int nscale, int minWaveLength, float mult, float sigmaOnf, int matchNum)
{
    _nscale = nscale,
...
cv::Mat1b LidarIris::GetIris(const pcl::PointCloud<pcl::PointXYZ> &cloud, Eigen::VectorXf &rowkey)
{
    int rows_ = 80, columns_ = 360;
    cv::Mat1b IrisMap = cv::Mat1b::zeros(rows_, columns_);
    ...
    rowkey = Eigen::VectorXf::Zero(rows_);
