...
// 用于存储range图五维信息(x,y,z,i,r)
struct PointXYZIRSCA
{
    PCL_ADD_POINT4D
    PCL_ADD_INTENSITY;
    float range;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIRSCA,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(float, range, range)(int, row_id, row_id)(int, col_id, col_id)(float, scan_position, scan_position)(float, curvature, curvature)(float, angle, angle))
...
#define distPt2Pt(a, b) sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y) + (a.z - b.z) * (a.z - b.z))
    ...
    public:
        ...
        void extractEdgePoint(pcl::PointCloud<VelodynePointXYZIRT>::Ptr pLaserCloudIn, ScanEdgePoints &edgePoints);
        ...
        void operator()(pcl::PointCloud<VelodynePointXYZIRT>::Ptr pLaserCloudIn,
                        vector<pcl::PointXYZI> &keyPoints,
                        cv::Mat &descriptors,
                        ScanEdgePoints &validCluster);

        void getCluster2(pcl::PointCloud<VelodynePointXYZIRT>::Ptr pLaserCloudIn, ScanEdgePoints &clusters);
        // void getCluster2(pcl::PointCloud<VelodynePointXYZIRT>::Ptr pLaserCloudIn, ScanEdgePoints &edgePoints);
