...
MatrixXd SCManager::makeRingkeyFromScancontext(Eigen::MatrixXd &_desc)
{
    ...
    {
        Eigen::MatrixXd curr_row = _desc.row(row_idx); // 将每一行的子矩阵取出
        invariant_key(row_idx, 0) = curr_row.mean();   // 对每一行的子矩阵求均值
    }
...
MatrixXd SCManager::makeSectorkeyFromScancontext(Eigen::MatrixXd &_desc)
{
    ...
    {
        Eigen::MatrixXd curr_col = _desc.col(col_idx); // 将每一列的子矩阵取出
        variant_key(0, col_idx) = curr_col.mean();     // 对每一列的子矩阵求均值
    }
...
// 输入自定义描述子
void SCManager::makeAndSaveScancontextAndKeys2(Eigen::MatrixXd &_sc)
{
    // Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(_sc);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(_sc);
    std::vector<float> polarcontext_invkey_vec = eig2stdvec(ringkey);

    polarcontexts_.push_back(_sc);
    polarcontext_invkeys_.push_back(ringkey);
    polarcontext_vkeys_.push_back(sectorkey);
    polarcontext_invkeys_mat_.push_back(polarcontext_invkey_vec);
...
Eigen::MatrixXd SCManager::getSC()
{
    return polarcontexts_.back();
}
